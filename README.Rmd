...
```
```{r echo = TRUE, eval = TRUE}
summary(fit, type="logOR", conf.int=0.95)
```
...
setDT(tableD1)
tableD1[, .(mean(est.sde), sd(est.sde), mean(se.sde), mean(est.sie), sd(est.sie), mean(se.sie), mean(est.oe), sd(est.oe), mean(se.oe)), by=.(n, n_bins)]
```
