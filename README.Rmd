...
Nuisance parameters can be modeled with any machine learning algorithm supported by the [<tt>`sl3`</tt>](https://github.com/tlverse/sl3) R package. To illustrate this we define a Super Learner for binary outcomes 
```{r echo = TRUE, eval = TRUE}
...
We use this learner to estimate the nuisance models $Q_Y$, $p_{R_Y}$ and $p_C$ by specifying the <tt>`Ylearner`</tt>, <tt>`RYlearner`</tt> and <tt>`Clearner`</tt> arguments as follows
```{r echo = TRUE, eval = TRUE}
...
We can also construct a super learner for the update steps $Q_L$ as follows
```{r echo = TRUE, eval = TRUE}
...
We use this learner to estimate $Q_L$ by specifying the <tt>`QLlearner`</tt> argument as follows
```{r echo = TRUE, eval = TRUE}
